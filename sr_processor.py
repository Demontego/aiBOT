...
def callback(ch, method, properties, body):
    ...
    opath[-1] = 'sr_' + opath[-1]
    opath = '/'.join(opath)
    Image.fromarray(rdn.predict(np.array(Image.open(message)))).save(opath + "_writing.png")
    shutil.move(opath + "_writing.png", opath)
    

connection = pika.BlockingConnection(pika.ConnectionParameters(heartbeat=0 ,socket_timeout=400000))
